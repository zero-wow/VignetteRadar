...
local function AddLabel(parent, title, x, y, verbatim)
    local label = parent:CreateFontString(nil, "OVERLAY", "GameFontNormal")
    label:SetPoint("TOPLEFT", parent, "TOPLEFT", x, y)
    label:SetFont(STANDARD_TEXT_FONT or "Fonts\\FRIZQT__.TTF", 10, "")
    label:SetTextColor(.68, .87, .81, 1)
    label:SetText(verbatim and title or addon.VignetteRadarControls.TitleCase(title))
    return label
end

local function AddDescription(parent, title, x, y, width)
    local label = AddLabel(parent, title, x, y, true)
...
local function AddCheckbox(parent, key, title, x, y, subkey, labelWidth, presentationOnly)
    ...
    label:SetText(addon.VignetteRadarControls.TitleCase(title))
...
local function AddButton(parent, title, x, y, width, callback)
    ...
    local button = (title == "+" or title == "-" or title == "−")
        and controls.IconButton(parent, title, width, 24)
        or controls.Button(parent, controls.TitleCase(title), width, 24)
...
local function BuildPanel()
    ...
    AddCheckbox(radar, "vignetteRadarHideWhenEmpty", "Hide Radar When There Are No Detections", 18, -146)
    ...
    AddLabel(layout, "Rounded Radar", 24, -118)
    AddDescription(layout, "Drag the radar edges to resize it. Its controls appear on hover.", 24, -150, 450)
    -- Preserve legacy layout widgets for old integrations without exposing
    -- the retired frame styles in the finalized settings view.
    AddLayoutChoice(layout, "classic", "Classic", 24, -148, 144):Hide()
    AddLayoutChoice(layout, "squat", "Squat", 188, -148, 144):Hide()
    AddLayoutChoice(layout, "compact", "Compact", 352, -148, 144):Hide()
    AddDescription(layout, "Current portrait radar with details below.", 24, -181, 144):Hide()
    AddDescription(layout, "Wide and short: radar left, details right, buttons below.", 188, -181, 144):Hide()
    AddDescription(layout, "Smaller radar with range and controls below.", 352, -181, 144):Hide()
    AddCheckbox(layout, "vignetteRadarNorthUp", "Keep North At The Top", 18, -229, nil, nil, true)
    AddCheckbox(layout, "vignetteRadarHoverTools", "Show Controls On Hover", 258, -229, nil, 205)
