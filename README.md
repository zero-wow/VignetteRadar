...
The short trail-mark icon in the footer is a quick travel-trail switch. Left-click to show or hide the trail; right-click to open a scrollable picker with animated examples of ten styles: **Dashes**, **Ticks**, **Dots**, **Long dashes**, **Slashes**, **Chevrons**, **Crosses**, **Diamonds**, **Beads**, and **Pulses**. Choosing a style turns the trail on. The same picker opens from **Styles & flow** in either Explore settings panel. Its spacing control sets the gap between marks (75–200%), Flow sets movement speed (Still, 0.5×, 1×, or 2×), and Fade keeps recent travel for 1, 3, or 5 minutes. These choices are saved. Dashes remain the default so the trail reads differently from quest dots.
